// WAV Audio Editor.cpp : Определяет точку входа для приложения.
//
#include <windows.h>
...
bool saveWavFile(std::wstring& fileName, const WAVEFORMATEXTENSIBLE& waveFormat, const std::vector<char>& data) {
    ...
    uint32_t riffChunkSize = 20 + fmtChunkSize + dataChunkSize; 
    ...
    file.write(data.data(), dataChunkSize);          
...
bool _loadFile(const wchar_t* filePath) {
    ...
    waveFormat.Format.nBlockAlign = blockAlign;            
    waveFormat.Format.nAvgBytesPerSec = byteRate;          
    ...
    case WM_COMMAND: {
        ...
        case RECORD:
            if (audioPlayerChannels.size() == 0) {
                ...
                if (nChannels < 1)
                    nChannels = 1;
                waveFormat.Format.wFormatTag = WAVE_FORMAT_EXTENSIBLE; 
                waveFormat.Format.nChannels = (WORD)nChannels;             
                waveFormat.Format.nSamplesPerSec = _targetDiscr;       
                waveFormat.Format.wBitsPerSample = bitsPerSample;      
                waveFormat.Format.nBlockAlign = bitsPerSample / 8 * nChannels;           
                waveFormat.Format.nAvgBytesPerSec = waveFormat.Format.nBlockAlign * _targetDiscr;
                waveFormat.Format.cbSize = sizeof(WAVEFORMATEXTENSIBLE) - sizeof(WAVEFORMATEX); 
                if (currentType == _FLOAT)
                    waveFormat.SubFormat = KSDATAFORMAT_SUBTYPE_IEEE_FLOAT;
                else
        ...
        case 18: {
            if (audioPlayerChannels.size() > 0) {
                ...
                waveFormat.Format.nSamplesPerSec = _targetDiscr;         
                waveFormat.Format.wBitsPerSample = typeSize * 8;      
